...
void get_way(pugi::xml_node *now)
{
    ...
    for (pugi::xml_node temp = now->first_child(); temp != now->last_child(); temp = temp.next_sibling())
    {
        if (strcmp(temp.name(),"nd") == 0)
        {
            ...
            {
                joint1 = points[points_map[temp.attribute("ref").as_uint()]];
                continue;
            }
            joint2 = points[points_map[temp.attribute("ref").as_uint()]];
...
void output()
{
    ...
    system("open /Users/caojingchen/proj/cpp/streetmap/Playground/shanghai_map.jpeg");
